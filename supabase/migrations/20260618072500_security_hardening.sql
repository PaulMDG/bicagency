-- 1. Remove public read on order_items (business data leak)
DROP POLICY IF EXISTS "public read order items" ON public.order_items;

-- 2. Remove public read on suppliers (internal contact data)
DROP POLICY IF EXISTS "public read active suppliers" ON public.suppliers;

-- 3. Remove permissive public INSERT — force going through SECURITY DEFINER place_order
...
-- 5. Lock down SECURITY DEFINER helper functions (internal use only)
REVOKE EXECUTE ON FUNCTION public.is_admin(uuid) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.has_role(uuid, public.app_role) FROM PUBLIC, anon, authenticated;

-- 6. Tighten EXECUTE grants on public RPCs
...
-- 7. Add safe RPC for confirmation-page payment polling (verified by phone)
...
BEGIN
  ...
  IF v_phone !~ '^254[17][0-9]{8}$' THEN RETURN jsonb_build_object('error','Invalid phone'); END IF;
...
-- 8. Protect sensitive settings from being marked public by mistake
...
-- Sweep any existing rows so the guard takes effect immediately
